        ...
        *,
        ...
        for prefix in ('s1', 's2', 's3', 's4'):
            ...
            layers.append(nn.Sequential(
                ...
                Transformer(dim = dim_next, depth = 1, heads=heads, local_patch_size = config['local_patch_size'], global_k = config['global_k'], dropout = dropout, has_local = not is_last),
                PEG(dim = dim_next, kernel_size = peg_kernel_size),
                Transformer(dim = dim_next, depth = config['depth'],  heads=heads, local_patch_size = config['local_patch_size'], global_k = config['global_k'], dropout = dropout, has_local = not is_last)
            ))
